...
typedef struct
{
    ...
    const void* (*output_pixels)(void* backend, uint32_t index, int32_t* stride);
...
// NULL if that output was not captured. `stride` receives bytes per row. Pixels
// are XRGB8888 little-endian, ready for a GL_BGRA upload.
